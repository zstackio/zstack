...
ALTER TABLE `zstack`.`ImageEO` ADD COLUMN `virtio` boolean NOT NULL DEFAULT TRUE;
UPDATE `zstack`.`ImageEO` SET virtio = FALSE, guestOsType="Other" WHERE platform="Windows";
UPDATE `zstack`.`ImageEO` SET platform = "Windows", virtio = TRUE, guestOsType = "Other" WHERE platform = "WindowsVirtio";
UPDATE `zstack`.`ImageEO` SET virtio = TRUE, guestOsType = "Other" WHERE platform = "Linux";
...
ALTER TABLE `zstack`.`VmInstanceEO` ADD COLUMN `guestOsType` varchar(255) NOT NULL;
DROP VIEW IF EXISTS `zstack`.`VmInstanceVO`;
