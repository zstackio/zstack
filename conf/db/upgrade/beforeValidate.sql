...
BEGIN
    ...
    THEN
        ...
        update `zstack`.`schema_version` set `checksum`=-1599310560  where `script`='V3.9.0.2__schema.sql' and `checksum` <> 1556501192;
        update `zstack`.`schema_version` set `checksum`=514679307  where `script`='V3.10.0__schema.sql' and `checksum` <> -762423017;
    END IF;
END $$
