...
public class EncryptManagerImpl extends AbstractService {
    ...
    public boolean start() {
        signedList = getAllSignedIntegrity();
        return true;
    }
    ...
    private void encryptedAllDateIntegrity(FlowTrigger trigger) {
        ...
        for (SignedColumn signedColumn : signedList) {
            ...
            String countSql = String.format("select count(1) from %s", signedColumn.getTableName());
            String selectSql = String.format("select %s, %s from %s",
            ...
            long count = SQL.New(countSql, Long.class).find();
            SQL.New(selectSql, Tuple.class).limit(1000).paginate(count, (List<Tuple> tuples, PaginateCompletion paginateCompletion) -> {
                new While<>(tuples).each((tuple, innerWhileCompletion) -> {
                    ...
                    if (!Q.New(EncryptionIntegrityVO.class)
                            .eq(EncryptionIntegrityVO_.resourceUuid, resourceUuid)
                            .eq(EncryptionIntegrityVO_.resourceType, signedColumn.getTableName())
                            .isExists()) {
                        String encryotValue = "";
