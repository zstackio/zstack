                    ...
                    flow(new NoRollbackFlow() {
                        ...
                        public void run(FlowTrigger trigger, Map data) {
                            ...
                            if (!KVMGlobalProperty.MN_NETWORKS.isEmpty()) {
                                ...
                                builder.append(String.format("; ipset create ZS-PROMETHEUS-ALLOW hash:net -exist; ipset flush ZS-PROMETHEUS-ALLOW; %s; iptables -t mangle -I INPUT -m set ! --match-set ZS-PROMETHEUS-ALLOW src -p tcp -m comment --comment %s -m tcp --dport 7069 -j DROP; iptables -t mangle -I INPUT -m set ! --match-set ZS-PROMETHEUS-ALLOW src -p tcp -m comment --comment %s -m tcp --dport 9092 -j DROP; iptables -t mangle -I INPUT -m set ! --match-set ZS-PROMETHEUS-ALLOW src -p tcp -m comment --comment %s -m tcp --dport 9100 -j DROP; iptables -t mangle -I INPUT -m set ! --match-set ZS-PROMETHEUS-ALLOW src -p tcp -m comment --comment %s -m tcp --dport 9103 -j DROP; iptables -t mangle -I INPUT -m set ! --match-set ZS-PROMETHEUS-ALLOW src -p tcp -m comment --comment %s -m tcp --dport 4900 -j DROP; ipset list | grep -w -q ZS-ALERTMANAGER-ALLOW && sudo iptables -t mangle -I INPUT -m set ! --match-set ZS-ALERTMANAGER-ALLOW src -p tcp -m comment --comment %s -m tcp --dport 9093 -j DROP",
                                        ipsetBuilder.toString(), KVMConstant.IPTABLES_COMMENTS, KVMConstant.IPTABLES_COMMENTS, KVMConstant.IPTABLES_COMMENTS, KVMConstant.IPTABLES_COMMENTS, KVMConstant.IPTABLES_COMMENTS, "alartmanager.allow.port"));
                            }
