...
public class KVMHost extends HostBase implements Host {
        ...
        compareCpuFuncSingleFlight.execute(
                ...
                new ReturnValueCompletion<CompareCpuFunctionOnHostReply>(msg) {
                    ...
                    public void success(CompareCpuFunctionOnHostReply returnValue) {
                        cfVO.setSupportLiveMigration(true);
                        dbf.persist(cfVO);
                    ...
                    public void fail(ErrorCode errorCode) {
                        ...
                        reply.setError(errorCode);
                        cfVO.setSupportLiveMigration(false);
                        dbf.persist(cfVO);
                        bus.reply(msg, reply);
                    }
                }
    ...
    public void deleteHook() {
        if (KVMGlobalConfig.ENABLE_VM_MIGRATION_HOST_CPU_FUNCTION_CHECK.value(Boolean.class)
                && Q.New(CpuFeaturesHistoryVO.class).eq(CpuFeaturesHistoryVO_.srcHostUuid, self.getUuid()).isExists()) {
    ...
    private void deleteCpuHistoryVOIfCpuModeNameChange(String originalModeName){
        //There are multiple records with the same srcHostUuid, as long as there is a cpu model name that is inconsistent, delete all the srcHostUuid records.
        String cpuModelName = Q.New(CpuFeaturesHistoryVO.class)
                .select(CpuFeaturesHistoryVO_.srcCpuModelName)
                .eq(CpuFeaturesHistoryVO_.srcHostUuid, self.getUuid())
                .findValue();
        if (!StringUtils.isEmpty(cpuModelName) && !cpuModelName.equals(originalModeName)) {
            SQL.New(CpuFeaturesHistoryVO.class).eq(CpuFeaturesHistoryVO_.srcHostUuid, self.getUuid()).delete();
        }
    }
