            ...
            if (!rsp.isSuccess()) {
                ...
                if (rsp.isFirstConnect()) {
                    ...
                    int drained = restf.failPendingCallsForResourceBefore(self.getUuid(), cutoffMillis,
                            operr(ORG_ZSTACK_CORE_REST_10013,
                                    "kvmagent on host[uuid:%s] restarted at %s, pending HTTP call sent before %s aborted",
    ...
    private void doUpdateVmConsolePasswordOnHypervisor(String vmUuid, String hostUuid, String password,
        ...
        bus.send(kmsg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (!rsp.isSuccess()) {
                    completion.fail(operr(ORG_ZSTACK_KVM_10133, rsp.getError()));
                } else {
