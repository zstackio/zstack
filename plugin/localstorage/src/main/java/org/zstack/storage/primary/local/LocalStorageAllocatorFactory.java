...
public class LocalStorageAllocatorFactory implements PrimaryStorageAllocatorStrategyFactory, Component,
    ...
    public String buildAllocatedInstallUrl(AllocatePrimaryStorageSpaceMsg msg, PrimaryStorageInventory psInv) {
        ...
        q.setParameter("psUuid", psInv.getUuid());
        ...
        if (!physicalCapacityMgr.checkCapacityByRatio(
            ...
            throw new OperationFailureException(operr("cannot reserve enough space for primary storage[uuid: %s]" +
                    " on host[uuid: %s], not enough physical capacity", psInv.getUuid(), hostUuid));
        }
        allocatedInstallUrl = ref.getHostUuid();
        if (allocatedInstallUrl==null){
            throw new CloudRuntimeException("---------------------------- allocatedInstallUrl is null ===============================");
        }
