...
public class LocalStorageAllocatorFactory implements PrimaryStorageAllocatorStrategyFactory, Component,
    ...
    private final static CLogger loggerls = Utils.getLogger(LocalStorageUtils.class);
    ...
    @Override
    public String getInstallPath(PrimaryStorageInventory psInv, AllocatePrimaryStorageMsg msg){
    ...
    public static void logCapacityChange(String psUuid, String hostUuid, long before, long after) {
        if (loggerls.isTraceEnabled()) {
            ...
            loggerls.trace(String.format("[Local Storage Capacity] %s:%s:%s changed the capacity of the local storage[uuid:%s], host[uuid:%s] as:\n" +
                            "available: %s --> %s\n",
