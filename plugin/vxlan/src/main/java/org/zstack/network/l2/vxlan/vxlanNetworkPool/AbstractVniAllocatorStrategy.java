...
public abstract class AbstractVniAllocatorStrategy implements VniAllocatorStrategy {
    ...
    protected Integer allocateRequiredVni(VniAllocateMessage msg) {
        ...
        if (vnir == null) {
            throw new OperationFailureException(Platform.err(ORG_ZSTACK_NETWORK_L2_VXLAN_VXLANNETWORKPOOL_10000, L2Errors.ALLOCATE_VNI_ERROR,
                    "cannot allocate vni[%s] in l2Network[uuid:%s], out of vni range", msg.getRequiredVni(), msg.getL2NetworkUuid()
            ));
        } else if (duplicate != null) {
            throw new OperationFailureException(Platform.err(ORG_ZSTACK_NETWORK_L2_VXLAN_VXLANNETWORKPOOL_10001, L2Errors.ALLOCATE_VNI_ERROR,
