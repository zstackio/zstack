...
class EnvSpec extends ApiHelper implements Node  {
    ...
    protected ConcurrentHashMap<Class, AtomicInteger> messageHandlerCounters = [:]
    ...
    void handleNoConditionSimulatorHttpRequests(HttpServletRequest req, Closure handler, HttpEntity entity, HttpServletResponse rsp) {
        ...
        try {
            ...
            httpHandlerCounters[url].incrementAndGet()
        }
    }
    ...
    boolean verifyAfterSimulator(String path) {
        return httpPostHandlerCounters[path] != null && httpPostHandlerCounters[path].get() > 0
    }

    boolean verifySimulator(String path) {
        return httpHandlerCounters[path] != null && httpHandlerCounters[path].get() > 0
    }

    boolean verifyMessage(Class clz) {
        return messageHandlerCounters[clz] != null && messageHandlerCounters[clz].get() > 0
    }
