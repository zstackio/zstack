...
class GoInventory implements SdkTemplate {
     ...
     * Should be called before each SDK generation run.
     */
    static void reset() {
    ...
    private void prepareApiTemplates() {
        ...
        allApiClasses.each { Class<?> apiClass ->
            ...
            try {
                ...
                // Only add valid templates (successfully initialized with @RestRequest)
                if (template.isValid()) {
    ...
    List<SdkFile> generate() {
        ...
        // Reset all static state for clean re-generation (fixes issue with repeated test runs)
        GoApiTemplate.reset()
        reset()
