...
> **仓库地址**: ssh://git@dev.zstack.io:9022/ye.zou/zstack-go-sdk.git
...
```bash
go get github.com/terraform-zstack-modules/zstack-sdk-go-v2
```
...
import (
    "fmt"
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/client"
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/param"
)
...
```go
// 查询所有虚拟机
vms, err := cli.QueryVmInstance(param.NewQueryParam())
if err != nil {
    panic(err)
}
...
```go
// 根据 UUID 获取虚拟机详情
vm, err := cli.GetVmInstance("vm-uuid-here")
if err != nil {
    panic(err)
}
...
```go
// 删除虚拟机（宽松模式）
err := cli.DestroyVmInstance("vm-uuid", param.Permissive)
if err != nil {
    panic(err)
}
```
...
```go
import "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/errors"

vm, err := cli.GetVmInstance("non-existent-uuid")
if err != nil {
...
import (
    ...
    // 3. 项目内部包
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/errors"
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/param"
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/view"
)
```
...
import (
    ...
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/param"
    "github.com/terraform-zstack-modules/zstack-sdk-go-v2/pkg/util/jsonutils"
)
...
func (cli *ZSClient) AddVmInstanceAsync(params param.CreateVmInstanceParam) (string, error) {
    var resp struct {
...
type BaseInfoView struct {
    ...
    Name *string `json:"name,omitempty"` // 资源名称（可选）
}
```
...
// 多参数：APIQueryGlobalConfigMsg (/global-configurations/{category}/{name})
func (cli *ZSClient) GetGlobalConfig(category string, name string) (*view.GlobalConfigInventoryView, error) {
    var resp view.GlobalConfigInventoryView
    // 使用 GetWithSpec：第一个参数作为 resourceId，其余构建 spec
    err := cli.GetWithSpec("v1/global-configurations", category, []string{name}, "", nil, &resp)
    if err != nil {
        return nil, err
    }
    return &resp, nil
...
```go
// ✅ 新方式
func (cli *ZSClient) Delete(resource string, uuid string, deleteMode string) error {
...
```go
// 旧方式
cli.Delete("v1/vm-instances/{uuid}", uuid, deleteMode)

// 新方式（更简洁）
cli.Delete("v1/vm-instances", uuid, deleteMode)
```
