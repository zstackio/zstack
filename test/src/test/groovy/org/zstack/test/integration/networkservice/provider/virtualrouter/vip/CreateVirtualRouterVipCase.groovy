 ...
 */
class CreateVirtualRouterVipCase extends SubCase{
    ...
    void test() {
        env.create {
            testltVirtualRouterPublicInterfaceVip()
        }
    }

    void testltVirtualRouterPublicInterfaceVip(){
        ...
        assert check
        VirtualRouterVmInventory inv = VirtualRouterVmInventory.valueOf(vrs.get(0))
        List<VipVO> vips = Q.New(VipVO.class).eq(VipVO_.ip, inv.getPublicNic().getIp()).list()
        assert vips.size() == 1

        destroyVmInstance {
            uuid = vrs.get(0).getUuid()
        }
        List<VipVO> vips1 = Q.New(VipVO.class).eq(VipVO_.ip, inv.getPublicNic().getIp()).list()
        assert vips1.size() == 0
    }
    @Override
    void clean() {
