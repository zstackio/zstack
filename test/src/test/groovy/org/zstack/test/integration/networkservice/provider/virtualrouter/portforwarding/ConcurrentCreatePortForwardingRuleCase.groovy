...
class ConcurrentCreatePortForwardingRuleCase extends SubCase {
    ...
    void testConcurrentCreateSamePortForwardingRule() {
        ...
        def formattedErrors = failures.collect { formatError(it.error) }
        assert failures.every { isVipPortConflictError(it.error, 8080, 8080) } :
                "duplicate failures should be VIP port conflict errors: errors=${formattedErrors}"
        ...
        assert isVipPortConflictError(lowerBoundaryConflict.error, 8079, 8080) :
        ...
        assert isVipPortConflictError(upperBoundaryConflict.error, 8080, 8081) :
    ...
    boolean isVipPortConflictError(org.zstack.sdk.ErrorCode error, int vipPortStart, int vipPortEnd) {
        return isPortForwardingOverlapError(error) ||
                isVipUsedPortRangeConflictError(error, vipPortStart, vipPortEnd)
    }

    boolean isPortForwardingOverlapError(org.zstack.sdk.ErrorCode error) {
        String details = error.details ?: ""
        return error.code == SysErrors.INVALID_ARGUMENT_ERROR.toString() &&
                details.contains("vipStartPort") &&
                details.contains("vipEndPort") &&
                details.contains("uuid:")
    }

    boolean isVipUsedPortRangeConflictError(org.zstack.sdk.ErrorCode error, int vipPortStart, int vipPortEnd) {
        String details = error.details ?: ""
        return error.code == SysErrors.OPERATION_ERROR.toString() &&
                details.contains("uuid:") &&
                details.contains(PortForwardingProtocolType.TCP.toString()) &&
                details.contains(vipPortStart.toString()) &&
                details.contains(vipPortEnd.toString())
    }

    String formatError(org.zstack.sdk.ErrorCode error) {
