 ...
 */
class CreateNfsDataVolumeFromTemplateFailedCase extends SubCase {
    EnvSpec env
    ...
    @Override
    void environment() {
        env = VolumeEnv.nfsStorageSftpEnv()
    }
    ...
    void prepare() {
        ps = env.inventoryByName("nfs-ps") as PrimaryStorageInventory
        kvm = env.inventoryByName("kvm") as KVMHostInventory
        image = env.inventoryByName("image-data-volume") as ImageInventory

        env.simulator(NfsPrimaryToSftpBackupKVMBackend.DOWNLOAD_FROM_SFTP_PATH) {
            def rsp = new NfsPrimaryStorageKVMBackendCommands.DownloadBitsFromSftpBackupStorageResponse()
