 ...
 */
class CreateLocalDataVolumeFromTemplateFailedCase extends SubCase {
    EnvSpec env
    ...
    @Override
    void environment() {
        env = VolumeEnv.localStorageSftpEnv()
    }
    ...
    void prepare() {
        ...
        env.simulator(LocalStorageKvmSftpBackupStorageMediatorImpl.DOWNLOAD_BIT_PATH) {
            def rsp = new LocalStorageKvmSftpBackupStorageMediatorImpl.SftpDownloadBitsRsp()
    ...
    void createVolumeFailed() {
        ...
        assert psGC.primaryStorageInstallPath == PathUtil.join(
                ps.url, "dataVolumes", "acct-" + currentEnvSpec.session.accountUuid, "vol-" + volumeUuid, volumeUuid + ".qcow2")
    }
}
