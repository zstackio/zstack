...
class SharedUdpIpvsLoadBalancerCase extends SubCase {
    EnvSpec env
    ...
    void test() {
        env.create {
            testSharedUdpIpvsRefreshPayload()
        }
    }
    ...
    void testSharedUdpIpvsRefreshPayload() {
        ...
        LoadBalancerServerGroupInventory sg = createLoadBalancerServerGroup {
            loadBalancerUuid = lb.uuid
            name = "shared-udp-ipvs-server-group"
        }
        ...
        VirtualRouterLoadBalancerBackend.RefreshLbCmd refreshCmd = null
        env.afterSimulator(VirtualRouterLoadBalancerBackend.REFRESH_LB_PATH) { rsp, HttpEntity<String> e ->
            refreshCmd = JSONObjectUtil.toObject(e.body, VirtualRouterLoadBalancerBackend.RefreshLbCmd.class)
            return rsp
        }
        ...
        retryInSecs {
            assert refreshCmd != null
            assert refreshCmd.lbs.size() == 1
        }

        VirtualRouterLoadBalancerBackend.LbTO to = refreshCmd.lbs[0]
        assert to.lbUuid == lb.uuid
        assert to.listenerUuid == listener.uuid
        assert to.mode == LoadBalancerConstants.LB_PROTOCOL_UDP
        ...
        assert to.loadBalancerPort == 19095
