...
class CreateDataVolumeAssignPsCase extends SubCase {
    ...
    void testBatchDeleteVolume() {
        ...
        def beforeCreateVolumePSAvailableCapacity = Q.New(PrimaryStorageCapacityVO.class)
                ...
                .eq(PrimaryStorageCapacityVO_.uuid, ls.uuid)
                .findValue()
        ...
        retryInSecs {
            assert Q.New(VolumeVO.class).count() == 20 + volCount
        }
        ...
        retryInSecs {
            assert beforeCreateVolumePSAvailableCapacity == afterCreateVolumePSAvailableCapacity + 20 * SizeUnit.GIGABYTE.toByte(500)
            assert beforeCreateVolumeHostAvailableCapacity == afterCreateVolumeHostAvailableCapacity + 20 * SizeUnit.GIGABYTE.toByte(500)
        }
        ...
        retryInSecs {
            assert Q.New(VolumeVO.class).eq(VolumeVO_.status, VolumeStatus.Deleted).count() == 20
        }
