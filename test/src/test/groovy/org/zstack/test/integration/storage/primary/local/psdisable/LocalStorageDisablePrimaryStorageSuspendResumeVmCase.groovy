 ...
 */
class LocalStorageDisablePrimaryStorageSuspendResumeVmCase extends SubCase{
    EnvSpec env
    ...
    void test() {
        env.create {
            testLocalStorageSuspendResumeVmWhenDisable()
        }
    }


    void testLocalStorageSuspendResumeVmWhenDisable() {
        ...
        KVMAgentCommands.ResumeVmCmd cmd1 = null
        env.afterSimulator(KVMConstant.KVM_RESUME_VM_PATH) { rsp, HttpEntity<String> e ->
            cmd1 = JSONObjectUtil.toObject(e.body, KVMAgentCommands.ResumeVmCmd.class)
            return rsp
        }
        resumeVmInstance {
            uuid = vmSpec.inventory.uuid
        ...
        assert cmd1 != null
        assert cmd1.uuid == vmSpec.inventory.uuid
        vmvo = dbFindByUuid(cmd.uuid, VmInstanceVO.class)
