...
import org.zstack.kvm.KVMAgentCommands.PingResponse
...
class KvmManagedComponentEndpointTest {
        ...
        try {
            ...
            withManagementServerIpProperties([:]) {
                ...
                RESTFacade restf = [
                        ...
                        buildSendCommandUrl : { String ignored -> sendCommandUrl }
                ] as RESTFacade
    ...
    @Test
    void endpointResolutionFailureDoesNotReportHostConfigurationMismatch() {
        withManagementServerIpProperties([
                "management.server.ip" : "192.168.1.10",
                "management.server.ip4": "192.168.1.10",
        ]) {
            KVMHostVO vo = new KVMHostVO()
            vo.uuid = Platform.uuid
            vo.managementIp = "2001:db8::20"
            KVMHostContext context = new KVMHostContext()
            context.baseUrl = "http://[2001:db8::20]:7070"
            KVMHost host = new KVMHost(vo, context)
            RESTFacade restf = [
                    getHostName      : { "192.168.1.10" },
                    getSendCommandUrl: { "http://192.168.1.10:8080/zstack/asyncrest/sendcommand" }
            ] as RESTFacade
            Field restFacadeField = KVMHost.class.getDeclaredField("restf")
            restFacadeField.setAccessible(true)
            restFacadeField.set(host, restf)
            PingResponse response = new PingResponse()
            response.sendCommandUrl = "http://192.168.1.10:8080/zstack/asyncrest/sendcommand"
            Method method = KVMHost.getDeclaredMethod("needUpdateHostConfiguration", PingResponse.class)
            method.accessible = true

            withErrorFacade {
                assert !(method.invoke(host, response) as boolean)
            }
        }
    }
