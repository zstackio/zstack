...
class Ipv6AddressAllocationCase extends SubCase {
    ...
    void testStateLessIpv6Allocation() {
        ...
        assert nic1.ip == IPv6Address.fromString(IPv6NetworkUtils.getIPv6AddresFromMac(ipr.getNetworkCidr(), nic1.mac)).toString()
        assert nic2.ip == IPv6Address.fromString(IPv6NetworkUtils.getIPv6AddresFromMac(ipr.getNetworkCidr(), nic2.mac)).toString()
        assert nic3.ip == IPv6Address.fromString(IPv6NetworkUtils.getIPv6AddresFromMac(ipr.getNetworkCidr(), nic3.mac)).toString()
    }
