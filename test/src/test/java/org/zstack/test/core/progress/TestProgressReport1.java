 ...
 *
 */
public class TestProgressReport1 {
    CLogger logger = Utils.getLogger(TestProgressReport1.class);
    Deployer deployer;
    Api api;
	...
	public void test() throws InterruptedException, ApiSenderException {
	    ...
	    for (int i=0; i<100; i++) {
            ...
            cmd1.setResourceUuid(resourceuuid);
            cmd1.setServerUuid(serveruuid);
            cmd1.setProgress("80%");
        ...
        String processType1 = "ChangeImage";
        String resourceuuid1 = image.getUuid();
        String serveruuid1 = host.getUuid();
        for (int i=0; i<100; i++) {
            ProgressReportCmd cmd = new ProgressReportCmd();
            cmd.setResourceUuid(resourceuuid1);
            cmd.setServerUuid(serveruuid1);
            cmd.setProgress("0%");
            cmd.setProcessType(processType1);
            cmd.setServerType(BackupStorage.class.getTypeName());
            restf.syncJsonPost(url, JSONObjectUtil.toJsonString(cmd), header, ProgressReportResponse.class);
            TimeUnit.MILLISECONDS.sleep(10);
            ...
            header = map(e(RESTConstant.COMMAND_PATH, ProgressConstants.PROGRESS_REPORT_PATH));
            ProgressReportCmd cmd1 = new ProgressReportCmd();
            cmd1.setResourceUuid(resourceuuid1);
            cmd1.setServerUuid(serveruuid1);
            cmd1.setProgress("60%");
            cmd1.setProcessType(processType1);
            cmd1.setServerType(BackupStorage.class.getTypeName());
            restf.syncJsonPost(url, JSONObjectUtil.toJsonString(cmd1), header, ProgressReportResponse.class);
            ...
            cmd2.setResourceUuid(resourceuuid1);
            cmd2.setServerUuid(serveruuid1);
            cmd2.setProgress("100%");
            cmd2.setProcessType(processType1);
        ...
        Assert.assertEquals("80%", vo.getProgress());


        TimeUnit.MILLISECONDS.sleep(2000);
        SimpleQuery<ProgressVO> q1 = dbf.createQuery(ProgressVO.class);
        q1.add(ProgressVO_.processType, SimpleQuery.Op.EQ, processType1);
        q1.add(ProgressVO_.resourceUuid, SimpleQuery.Op.EQ, resourceuuid1);
