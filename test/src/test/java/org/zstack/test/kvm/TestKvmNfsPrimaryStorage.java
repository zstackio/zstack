...
public class TestKvmNfsPrimaryStorage {
    static CLogger logger = Utils.getLogger(TestKvmNfsPrimaryStorage.class);
    ...
    public void testAttach() throws ApiSenderException, InterruptedException {
        addHost();
        nfsConfig.totalCapacity = SizeUnit.GIGABYTE.toByte(100);
        nfsConfig.availableCapacity = SizeUnit.GIGABYTE.toByte(50);
        PrimaryStorageInventory pinv = addPrimaryStorage();
        TimeUnit.SECONDS.sleep(2);
        PrimaryStorageVO pvo = dbf.findByUuid(pinv.getUuid(), PrimaryStorageVO.class);
        Assert.assertEquals(nfsConfig.totalCapacity, pvo.getCapacity().getTotalCapacity());
        Assert.assertEquals(nfsConfig.availableCapacity, pvo.getCapacity().getAvailableCapacity());
        String mountBase = NfsPrimaryStorageGlobalConfig.MOUNT_BASE.value(String.class);
        String mountPath = PathUtil.join(mountBase, "prim-" + pinv.getUuid());
        Assert.assertEquals(mountPath, pvo.getMountPath());
    }
}
