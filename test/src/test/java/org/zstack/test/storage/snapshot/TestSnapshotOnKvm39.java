...
*
* confirm full rebase is executed and new snapshot create success
*/
public class TestSnapshotOnKvm39 {
    CLogger logger = Utils.getLogger(TestSnapshotOnKvm39.class);
    Deployer deployer;
    Api api;
    ...
    public void test() throws ApiSenderException {
        ...
        api.deleteSnapshot(root.getUuid());
        Assert.assertEquals(1, kvmConfig.mergeSnapshotCmds.size());
        MergeSnapshotCmd cmd = kvmConfig.mergeSnapshotCmds.get(0);
        Assert.assertTrue(cmd.isFullRebase());

        snapshotKvmSimulator.validateNotExisting(root.getPrimaryStorageInstallPath());

        root = api.createSnapshot(volUuid);
