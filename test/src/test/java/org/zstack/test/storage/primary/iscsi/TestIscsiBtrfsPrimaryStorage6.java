...
public class TestIscsiBtrfsPrimaryStorage6 {
    ...
    public void test() throws ApiSenderException, InterruptedException {
        ...
        Assert.assertEquals(1, iconfig.createSymlinkCmds.size());

        Assert.assertEquals(BootDev.cdrom.toString(), scmd.getBootDev());
        Assert.assertEquals(String.format("http://%s/iso/%s.iso", iscsi.getHostname(), iso.getUuid()), scmd.getBootIsoPath());
    }
}
