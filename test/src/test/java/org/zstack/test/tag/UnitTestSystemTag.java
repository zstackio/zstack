...
public class UnitTestSystemTag {
    ...
    public void mockCreate(String resUuid){
        ...
        TypedQuery<SystemTagVO> typedQuery = Mockito.mock(TypedQuery.class);

        Mockito.when(entityManager.createQuery("select t from SystemTagVO t where t.uuid = :uuid", SystemTagVO.class)).thenAnswer(new Answer<TypedQuery<SystemTagVO>>() {
            @Override
            public TypedQuery<SystemTagVO> answer(InvocationOnMock invocationOnMock) throws Throwable {
                return typedQuery;
            }
        });
        ...
        Mockito.when(typedQuery.getSingleResult()).thenReturn(systemTagVO);
    }
