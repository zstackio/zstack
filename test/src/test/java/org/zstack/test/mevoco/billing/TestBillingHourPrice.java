 ...
 */
public class TestBillingHourPrice {
    CLogger logger = Utils.getLogger(TestBillingHourPrice.class);
    Deployer deployer;
    Api api;
    ...
    public void test() throws ApiSenderException, InterruptedException {
        ...
        long duringSeconds = 5;
        api.startVmInstance(vm.getUuid());
        TimeUnit.SECONDS.sleep(duringSeconds);
        api.stopVmInstance(vm.getUuid());
        ...
        float cpuPrice = vm.getCpuNum() * 100f * duringSeconds / 3600;
        float memPrice = vm.getMemorySize() * 10f * duringSeconds / 3600;
        long volSizeInM = SizeUnit.BYTE.toMegaByte(rootVolume.getSize());
        float volPrice = volSizeInM * 9f * duringSeconds / 3600;
        ...
        Spending spending = CollectionUtils.find(reply.getSpending(),
                arg -> BillingConstants.SPENDING_TYPE_VM.equals(arg.getSpendingType()) ? arg : null);
        Assert.assertNotNull(spending);
        List<VmSpending> vmSpendings = JSONObjectUtil.toCollection(
                JSONObjectUtil.toJsonString(spending.getDetails()), ArrayList.class, VmSpending.class);
        ...
        spending = CollectionUtils.find(reply.getSpending(),
                arg -> BillingConstants.SPENDING_TYPE_ROOT_VOLUME.equals(arg.getSpendingType()) ? arg : null);
        Assert.assertNotNull(spending);
