...
public class VolumeSnapshotManagerImpl extends AbstractService implements
    ...
    private void handle(GetVolumeSnapshotEncryptedMsg msg) {
        ...
        encryptedMsg.setSnapshotUuid(snapshotVO.getUuid());
        encryptedMsg.setPrimaryStorageUuid(snapshotVO.getPrimaryStorageUuid());
        encryptedMsg.setPrimaryStorageInstallPath(snapshotVO.getPrimaryStorageInstallPath());
        bus.makeTargetServiceIdByResourceUuid(encryptedMsg, PrimaryStorageConstant.SERVICE_ID, snapshotVO.getPrimaryStorageUuid());
        ...
        chain.then(new ShareFlow() {
            ...
            public void setup() {
                ...
                flow(new NoRollbackFlow() {
                    ...
                    @Override
                    public void run(FlowTrigger trigger, Map data) {
                        GetVolumeSnapshotEncryptedMsg encryptedMsg = new GetVolumeSnapshotEncryptedMsg();
                        encryptedMsg.setSnapshotUuid(struct.getCurrent().getUuid());
                        encryptedMsg.setPrimaryStorageUuid(snapshot.getPrimaryStorageUuid());
                        encryptedMsg.setPrimaryStorageInstallPath(snapshot.getPrimaryStorageInstallPath());
                        bus.makeTargetServiceIdByResourceUuid(encryptedMsg, VolumeSnapshotConstant.SERVICE_ID, primaryStorageUuid);
                        bus.send(encryptedMsg, new CloudBusCallBack(trigger) {
                            @Override
                            public void run(MessageReply reply) {
                                if (!reply.isSuccess()) {
                                    trigger.fail(reply.getError());
