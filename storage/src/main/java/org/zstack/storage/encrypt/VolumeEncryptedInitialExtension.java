...
/**
 * Encrypted volume instantiate: {@link #preInstantiateVolume} prepares host LUKS secret material file;
 * {@link #afterInstantiateVolume} defines the libvirt secret on the host (needs DEK again after async PS step).
 */
...
public class VolumeEncryptedInitialExtension implements PreInstantiateVolumeExtensionPoint,
    ...
    public void preInstantiateVolume(InstantiateVolumeMsg msg) {
        ...
        msg.setVolumeLuksAgentSpec(spec);
        ...
        if (volume != null && volume.isEncrypted()) {
            ...
            String secFilePath = secretHelper.ensureLuksSecretFileOnHost(hostUuid, volUuid, dekBase64);
            spec.setEncryptLuksSecretMaterialFilePath(secFilePath);
        }
    }
    ...
    public void afterInstantiateVolume(InstantiateVolumeOnPrimaryStorageMsg msg) {
        ...
        VolumeLuksAgentSpec spec = msg.getVolumeLuksAgentSpec();
        if (spec == null || StringUtils.isBlank(spec.getEncryptLuksSecretMaterialFilePath())) {
            return;
        }
        if (StringUtils.isNotBlank(spec.getEncryptSecretUuid())) {
            return;
        }
        ...
        String kpUuid = volumeEncryptedResourceKeyBackend.findKeyProviderUuidByVolume(volUuid);
        String libvirtSecretUuid = secretHelper.defineSecretFromBinding(
                destHost.getUuid(), vmInstanceUuid, volUuid, kpUuid);

        spec.setEncryptSecretUuid(libvirtSecretUuid);
    }
