...
public class VolumeSnapshotEncryptionHelper {
    ...
    public VolumeLuksAgentSpec prepareVolumeSecretMaterial(String hostUuid, String volumeUuid) {
        ...
        if (volume == null) {
            throw new OperationFailureException(operr("target volume[uuid:%s] not found", volumeUuid));
        }
        if (!volume.isEncrypted()) {
            return null;
        }

        String kpUuid = keyBackend.findKeyProviderUuidByVolume(volume.getUuid());
        if (StringUtils.isBlank(kpUuid)) {
            throw new OperationFailureException(operr(
                    "encrypted volume[uuid:%s] has no key provider binding", volume.getUuid()));
        }

        EncryptedResourceKeyManager.GetOrCreateResourceKeyContext ctx =
                new EncryptedResourceKeyManager.GetOrCreateResourceKeyContext();
        ctx.setResourceUuid(volume.getUuid());
        ctx.setResourceType(VolumeVO.class.getSimpleName());
        ctx.setKeyProviderUuid(kpUuid);
        ctx.setPurpose("prepare-volume-secret-material");
        ...
        if (StringUtils.isBlank(keyResult.getDekBase64())) {
            throw new OperationFailureException(operr(
                    "key manager returned empty DEK for encrypted volume[uuid:%s]", volume.getUuid()));
        }

        VolumeLuksAgentSpec spec = new VolumeLuksAgentSpec();
        spec.setEncryptLuksSecretMaterialFilePath(
                secretHelper.ensureLuksSecretFileOnHost(hostUuid, volume.getUuid(), keyResult.getDekBase64()));
        return spec;
    }
