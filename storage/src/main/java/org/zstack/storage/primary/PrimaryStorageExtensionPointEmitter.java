	...
	void preDelete(PrimaryStorageInventory inv) throws PrimaryStorageException {
		for (PrimaryStorageDeleteExtensionPoint extp : delExts) {
			try {
                ...
                logger.debug(String.format("%s refused to delete primary storage[uuid:%s] because %s", extp.getClass().getName(), inv.getUuid(), pe.getMessage()));
                throw pe;
			} catch (Exception e) {
	...
	void preChange(PrimaryStorageVO vo, PrimaryStorageStateEvent evt) throws PrimaryStorageException {
		...
		for (PrimaryStorageChangeStateExtensionPoint extp : changeExts) {
			try {
                ...
                logger.debug(String.format("%s refused to change primary storage[uuid:%s] state from %s to %s because %s", extp.getClass().getCanonicalName(), vo.getUuid(), vo.getState(), next, pe.getMessage()), pe);
                throw pe;
			} catch (Exception e) {
	...
	void preAttach(PrimaryStorageVO vo, String clusterUuid) throws PrimaryStorageException {
		...
		for (PrimaryStorageAttachExtensionPoint extp : attachExts) {
			try {
                ...
                logger.debug(String.format("%s refused to attach primary storage[uuid:%s] because %s", extp.getClass().getName(), inv.getUuid(), pe.getMessage()));
                throw pe;
            } catch (Exception e) {
	...
	String preDetach(PrimaryStorageVO vo, String clusterUuid) throws PrimaryStorageException {
		...
		for (PrimaryStorageDetachExtensionPoint extp : detachExts) {
			try {
                ...
                logger.debug(String.format("%s refused to detach primary storage[uuid:%s, name: %s] because %s", extp.getClass().getName(), inv.getUuid(), inv.getName(), pe.getMessage()));
                throw pe;
			} catch (Exception e) {
