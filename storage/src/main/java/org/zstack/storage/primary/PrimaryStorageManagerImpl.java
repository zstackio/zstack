...
public class PrimaryStorageManagerImpl extends AbstractService implements PrimaryStorageManager,
    ...
    private void handle(ReleasePrimaryStorageSpaceMsg msg) {
        ...
        PSReserveCapacityExt.releaseCapacity(msg.getAllocatedInstallUrl(), msg.getDiskSize(), msg.getPrimaryStorageUuid());
    }
    ...
    private void allocatePrimaryStoreSpace(AllocatePrimaryStorageSpaceMsg msg, NoErrorCompletion completion) {
        ...
        while (it.hasNext()) {
            ...
            if (reserveSpace(psInv, requiredSize, msg, allocatorStrategyType) != null) {
                target = psInv;
                break;
            } else {
        ...
        reply.setAllocatedInstallUrl(msg.getRequireAllocatedInstallUrl());
        bus.reply(msg, reply);
        completion.done();
    }
        ...
        updater.run(new PrimaryStorageCapacityUpdaterRunnable() {
            ...
            public PrimaryStorageCapacityVO call(PrimaryStorageCapacityVO cap) {
                ...
                if (PSCapacityExt == null) {
                    String errInfo = String.format("Don't deal Primary Storage type[type: %s] ", inv.getType());
                    throw new CloudRuntimeException(errInfo);
                }
                installUrl[0] = PSCapacityExt.reserveCapacity(PSCapacityExt.buildAllocatedInstallUrl(msg, msg.getRequiredPrimaryStorageUuid()), msg.getSize(), msg.getRequiredPrimaryStorageUuid());
                return cap;
            }
        });
    ...
    public boolean start() {
        ...
        pluginRgty.saveExtensionAsMap(PSCapacityExtensionPoint.class, new Function<Object, PSCapacityExtensionPoint>() {
            ...
            public Object call(PSCapacityExtensionPoint arg) {
                return arg.psCapacityPrimaryStorageType();
            }
        });
        return true;
    }
