    ...
    public void allocate() {
        ...
        if (ret.isEmpty()) {
            if (CollectionUtils.isEmpty(candidates)) {
                fail(operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10028, "no host having state=Enabled status=Connected hypervisorType=%s found",
            ...
            if (isNoConnectedHost()) {
                error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10029, "no Connected hosts found in the [%s] candidate hosts", candidates.size());
            } else if (isNoEnabledHost()) {
                error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10030, "no Enabled hosts found in the [%s] candidate hosts", candidates.size());
                ...
                if (spec.getHypervisorType() != null) {
                    ...
                    error = operr(ORG_ZSTACK_COMPUTE_ALLOCATOR_10033, "no host having state=Enabled status=Connected found");
                }
            }
