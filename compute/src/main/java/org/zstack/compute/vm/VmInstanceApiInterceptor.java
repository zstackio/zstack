...
public class VmInstanceApiInterceptor implements ApiMessageInterceptor {
    ...
    private void validate(APIUpdateVmInstanceMsg msg) {
        ...
        Tuple result = SQL.New(
                "select sum(hc.totalCpu), sum(hc.availableCpu), sum(hc.availableMemory), sum(hc.totalMemory)" +
                ...
                .find();
        Long availableCpu = (Long) result.get(1);
        Long availableMemory = (Long) result.get(2);
        ...
        if (results.size() == 3) { //Enterprise Edition
            ...
            Double totalAvailablMemory = (availableMemory - reservedMemory) * overProvisioningMemory;
            Double totalAvailableCpu = availableCpu * overProvisioningCpu;
            if ((requestCpu != null && requestCpu > totalAvailableCpu.intValue() ) || (requestMemory != null && requestMemory > totalAvailablMemory.longValue() )) {
            ...
            Long reservedMemory = SizeUtils.sizeStringToBytes(results.get(0));
            Long totalAvailablMemory = availableMemory - reservedMemory;
            Long totalAvailabllCpu = availableCpu ;
