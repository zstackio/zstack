...
public class VmAllocateNicFlow implements Flow {
    ...
    public void run(final FlowTrigger trigger, final Map data) {
        ...
        new While<>(spec.getL3Networks()).each((nw, wcomp) -> {
            ...
            String customMac = new MacOperator().getMac(spec.getVmInventory().getUuid(), nw.getUuid());
            if (customMac != null){
                new MacOperator().deleteCustomMacSystemTag(spec.getVmInventory().getUuid(), nw.getUuid(), customMac);
