...
public class VmAllocateHostAndPrimaryStorageFlow implements Flow {
    ...
    public void run(final FlowTrigger trigger, final Map data) {
        ...
        List<List<String>> sortedPsGroup = getSortedPrimaryStorageGroups(data, spec);
    ...
    private List<List<String>> getSortedPrimaryStorageGroups(final Map data, VmInstanceSpec spec) {
        ...
        Map<List<String>, List<String>> psGroupsMapToClusterGroups = getPsGroupsMapToClusterGroups(sortedClusters, spec);

        List<List<String>> psGroups = new ArrayList<>();
        List<List<String>> clusterGroups = new ArrayList<>();
        for (Map.Entry<List<String>, List<String>> entry : psGroupsMapToClusterGroups.entrySet()) {
            psGroups.add(entry.getKey());
            clusterGroups.add(entry.getValue());
        }

        if (!isNeedAutoAllocatePrimaryStorage(psGroups)) {
            return Collections.emptyList();
        }

        List<List<String>> sortedPsGroup = new ArrayList<>();
        sortedClusters.forEach(cluster -> clusterGroups.forEach(clusterList -> {
            if (clusterList.contains(cluster)) {
                List<String> psList = psGroups.get(clusterGroups.indexOf(clusterList));
    ...
    private Map<List<String>, List<String>> getPsGroupsMapToClusterGroups(List<String> sortedClusters, VmInstanceSpec spec) {
        ...
        sortedClusters.forEach(clusterUuid -> {
            List<String> psList = getPrimaryStoragesFromClusters(clusterUuid);
            if (psList.size() != 0) {
                clusterAccessiblePS.put(clusterUuid, psList);
            }
        });

        Map<List<String>, List<String>> psGroupsMapToClusterGroups = new HashMap<>();
        clusterAccessiblePS.forEach((key, value) -> {
            if (psGroupsMapToClusterGroups.get(value) == null) {
                psGroupsMapToClusterGroups.put(value, new ArrayList<>(Collections.singleton(key)));
            } else {
                psGroupsMapToClusterGroups.get(value).add(key);
            }
        });
    ...
    private List<String> getPrimaryStoragesFromClusters(String clusterUuid) {
        return Q.New(PrimaryStorageClusterRefVO.class).select(PrimaryStorageClusterRefVO_.primaryStorageUuid)
    ...
    private boolean isNeedAutoAllocatePrimaryStorage(List<List<String>> psGroup) {
        return psGroup.stream().map(this::getPrimaryStorageTypes).filter(this::filterRules).count() == psGroup.size();
    }

    private List<String> getPrimaryStorageTypes(List<String> psUuids) {
        return Q.New(PrimaryStorageVO.class).select(PrimaryStorageVO_.type).in(PrimaryStorageVO_.uuid, psUuids).listValues();
    }
