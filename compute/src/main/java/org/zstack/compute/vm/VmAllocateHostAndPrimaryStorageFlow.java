...
public class VmAllocateHostAndPrimaryStorageFlow implements Flow {
                ...
                flow(new NoRollbackFlow() {
                    ...
                    public void run(FlowTrigger trigger, Map data) {
                        ...
                        if (availablePsTuples.isEmpty()) {
                            errorCodesOut.add(operr("no available ps"));
                        ...
                        if (autoAllocateRootVolumePs && autoAllocateDataVolumePs) {
                            ...
                            new While<>(psCombos).each((psCombo, whileCompletionInn) -> {
                                ...
                                chain.done(new FlowDoneHandler(whileCompletionInn) {
                                    @Override
                                    public void handle(Map data) {
                                        whileCompletionInn.allDone();
                                    }
                                }).error(new FlowErrorHandler(whileCompletionInn) {
                                    @Override
                        ...
                        new While<>(availablePsUuids).each((psUuid, whileCompletionInn) -> {
                            if (autoAllocateRootVolumePs) {
    ...
    class PSAndSClusterGroup {
        PSAndSClusterGroup(List<ArrayList<String>> psGroup, List<ArrayList<String>> clusterGroup) {
            this.psGroup = psGroup;
            this.clusterGroup = clusterGroup;
        }

        List<ArrayList<String>> psGroup;
        List<ArrayList<String>> clusterGroup;
    }
}
