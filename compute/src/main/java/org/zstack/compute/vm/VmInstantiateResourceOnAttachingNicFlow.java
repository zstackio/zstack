...
public class VmInstantiateResourceOnAttachingNicFlow implements Flow {
    ...
    private void releaseResource(final Iterator<InstantiateResourceOnAttachingNicExtensionPoint> it, final VmInstanceSpec spec, final L3NetworkInventory l3, final FlowTrigger trigger) {
        if (!it.hasNext()) {
            trigger.rollback();
            return;
        }
