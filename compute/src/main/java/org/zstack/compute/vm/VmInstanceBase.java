...
public class VmInstanceBase extends AbstractVmInstance {
    ...
    protected void executeCrashStrategy(final Message fmsg, final Completion completion) {
        ...
        if (crashStrategy.equals(CrashStrategy.Shutdown.toString())) {
            ...
            if (!msg.getNeedToReboot()) {
                logger.info("try to reboot vm exceeds threshold, prevent rebooting");
                completion.success();
                return;
            }
            StopVmInstanceMsg smsg = new StopVmInstanceMsg();
            ...
            stopVm(smsg, new Completion(completion) {
                @Override
                public void success() {
                    logger.info(String.format("stop the vm[uuid:%s] on the host[uuid:%s]", vmInv.getUuid(), vmInv.getHostUuid()));
                    StartVmInstanceMsg smsg = new StartVmInstanceMsg();
                    smsg.setVmInstanceUuid(vmInv.getUuid());
                    startVm(smsg, new Completion(completion) {
                        @Override
                        public void success() {
                            logger.info(String.format("start the vm[uuid:%s] on the host[uuid:%s]", vmInv.getUuid(), vmInv.getHostUuid()));
                            completion.success();
                        }
